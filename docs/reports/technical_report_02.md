...
> **Editorial note (post-report reorganization).** At the time of this
> report, `aether_api` (§4-§5) lived in this repository. It has since
> been extracted to its own repository,
> [`Manifestro/aether-api`](https://github.com/Manifestro/aether-api),
> which depends on this repository's core as a versioned package (see
> `docs/plan.md` §1 for the product/research split rationale). The
> experiments, code excerpts, and file paths below are preserved exactly
> as run and are still accurate history; they no longer describe this
> repository's current file layout.
