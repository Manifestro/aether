...
5. [`notebooks/aether_stage5_hidden_state_voice_colab.ipynb`](../notebooks/aether_stage5_hidden_state_voice_colab.ipynb) —
   ...
   **hidden state Qwen3**, не из текста, на ~20 английских фразах, с `kyutai/tts-1.6b-en_fr` как
   ...
   Для train-фраз декодируются два `.wav` на каждую (`predicted` — выход обученной головы,
   `teacher-codebook0-only` — тот же учитель, но через тот же единственный codebook, что и
   предсказывает голова, — это потолок качества для сравнения, а не полный TTS-звук).
