...
- **Методологическая ошибка, найденная и исправленная после первого Stage 5 прослушивания.**
  Первая версия decode-шага раскладывала звук только через codebook 0 (тот единственный, что
  предсказывает `HiddenStateVoiceHead`) — и для "predicted", и для эталона от учителя. Оба звучали
  неотличимым шумом, потому что Mimi специально разносит смысловое содержание (codebook 0) и
  акустическую детализацию (codebooks 1-31): без последних разборчивой речи не получить, даже у
  настоящего учителя. Это не было доказательством поломки пайплайна — decode только через один
  codebook в принципе не даёт материала для честного сравнения на слух. Раннер переписан: теперь
  декодируется полный 32-кодбуковый голос учителя (`teacher_voice_full`) и гибрид с подменённым
  на предсказание головы codebook 0 (`hybrid_our_codebook0`) — см. §"Что прислать для анализа".
